...
def build_matchup_features(pitches: pd.DataFrame, batter_id: int, pitcher_id: int, before: pd.Timestamp) -> dict[str, float]:
    ...
    merged = mix.merge(batter, on="pitch_type", how="left")
    if merged.empty:
        return {"pitch_matchup_hr_rate": float("nan"), "pitch_matchup_hard_hit_rate": float("nan"), "pitch_matchup_barrel_rate": float("nan"), "pitch_mix_coverage": 0.0}
    return {
        "pitch_matchup_hr_rate": _weighted_mean(merged["batter_hr_rate"], merged["usage"]),
