...
from .daily_board import (
    _add_matchup_context,
    _add_park_context,
...
WEIGHTS = {
    "production_current": (0.70, 0.15, 0.10, 0.05),
    "baseline_only": (1.00, 0.00, 0.00, 0.00),
    "baseline_80_matchup_15_weather_5": (0.80, 0.15, 0.00, 0.05),
    "baseline_75_matchup_15_park_10": (0.75, 0.15, 0.10, 0.00),
    "baseline_65_matchup_20_park_10_weather_5": (0.65, 0.20, 0.10, 0.05),
    "baseline_60_matchup_25_park_10_weather_5": (0.60, 0.25, 0.10, 0.05),
    "baseline_50_matchup_25_park_15_weather_10": (0.50, 0.25, 0.15, 0.10),
}


def _training_cutoffs(history: pd.DataFrame) -> tuple[pd.Series, pd.Series]:
    dates = pd.to_datetime(history["game_date"])
    return dates.quantile(0.70), dates.quantile(0.85)


def _build_day_board(raw, history, model, features, day: date) -> pd.DataFrame:
    ...
    historical_before_day = raw[raw["game_date"] < day.isoformat()]
    board = _add_matchup_context(board, historical_before_day, day)
    board = _add_park_context(board)
    board = _add_weather_context(board, games, day)
    return board


def _score_weights(board: pd.DataFrame, weights: tuple[float, float, float, float]) -> pd.Series:
    baseline = board["hr_probability"].rank(pct=True)
    matchup = board["matchup_score"].rank(pct=True).fillna(0.5)
    park = board["park_score"].fillna(0.5)
    weather = board["weather_score"].fillna(0.5)
    return weights[0] * baseline + weights[1] * matchup + weights[2] * park + weights[3] * weather


def _daily_metrics(board: pd.DataFrame, actuals: pd.Series, name: str) -> dict:
    slate = board.assign(actual_hr=actuals)
    slate_rate = float(slate["actual_hr"].mean())
    row: dict[str, float | int | str] = {
        "weight_set": name,
        "players": len(slate),
        "slate_hr_rate": slate_rate,
    }
    for k in (5, 10, 20, 50, 100):
        top = slate.sort_values("ranking_score", ascending=False).head(min(k, len(slate)))
...
def run_backtest(raw_csv: Path, start: date, end: date) -> pd.DataFrame:
    ...
    for day_offset in range((end - start).days + 1):
        ...
        control = board.copy()
        control["ranking_score"] = control["hr_probability"]
        rows.append(_daily_metrics(control, actuals, "model_probability_control") | {"date": day.isoformat()})
...
def main() -> None:
    ...
    summary = result.groupby("weight_set").agg(
        ...
        top100_lift=("top100_lift", "mean"),
        top5_hr=("top5_hr", "sum"),
        top10_hr=("top10_hr", "sum"),
        top20_hr=("top20_hr", "sum"),
        top50_hr=("top50_hr", "sum"),
        top100_hr=("top100_hr", "sum"),
